-- MANUAL_MIGRATION_COMBINED.sql
-- Объединенные миграции для ручного применения в Supabase SQL Editor
-- Инструкция: скопируйте и выполните в Dashboard -> SQL Editor

-- =============================================================================
-- МИГРАЦИЯ 1: job_postings table (20250913120000)
-- =============================================================================

-- Создание основных справочных таблиц если их нет
...
-- Добавляем недостающие столбцы если их нет (для существующих таблиц)
DO $$ 
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
    ...
    ADD COLUMN status text CHECK (status IN ('draft', 'active', 'paused', 'filled', 'expired')) DEFAULT 'active';
  END IF;
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
    ...
    ALTER TABLE public.job_postings 
    ADD COLUMN category text CHECK (category IN ('service', 'kitchen', 'management', 'cleaning', 'other')) DEFAULT 'service';
  END IF;

  -- Добавляем столбец position_type
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_schema = 'public' 
                   AND table_name = 'job_postings' 
                   AND column_name = 'position_type') THEN
    ALTER TABLE public.job_postings 
    ADD COLUMN position_type text CHECK (position_type IN ('waiter', 'bartender', 'cook', 'chef', 'cashier', 'cleaner', 'manager', 'host', 'other')) DEFAULT 'waiter';
  END IF;

  -- Добавляем столбец employment_type
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_schema = 'public' 
                   AND table_name = 'job_postings' 
                   AND column_name = 'employment_type') THEN
    ALTER TABLE public.job_postings 
    ADD COLUMN employment_type text CHECK (employment_type IN ('full_time', 'part_time', 'temporary', 'internship')) DEFAULT 'full_time';
  END IF;

  -- Добавляем столбец schedule_type
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_schema = 'public' 
                   AND table_name = 'job_postings' 
                   AND column_name = 'schedule_type') THEN
    ALTER TABLE public.job_postings 
    ADD COLUMN schedule_type text CHECK (schedule_type IN ('fixed', 'flexible', 'shift', 'night', 'weekend')) DEFAULT 'shift';
  END IF;

  -- Добавляем столбец salary_min
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_schema = 'public' 
                   AND table_name = 'job_postings' 
  ...
  -- Добавляем столбец salary_type
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_schema = 'public' 
                   AND table_name = 'job_postings' 
                   AND column_name = 'salary_type') THEN
    ALTER TABLE public.job_postings 
    ADD COLUMN salary_type text CHECK (salary_type IN ('hourly', 'daily', 'monthly')) DEFAULT 'monthly';
  END IF;

  -- Добавляем столбец currency
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_schema = 'public' 
                   AND table_name = 'job_postings' 
  ...
  -- Добавляем столбец applications_count
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_schema = 'public' 
                   AND table_name = 'job_postings' 
                   AND column_name = 'applications_count') THEN
    ALTER TABLE public.job_postings 
    ADD COLUMN applications_count integer DEFAULT 0;
  END IF;

  -- Добавляем массивы если их нет
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_schema = 'public' 
                   AND table_name = 'job_postings' 
                   AND column_name = 'required_skills') THEN
    ALTER TABLE public.job_postings 
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_schema = 'public' 
                   AND table_name = 'job_postings' 
                   AND column_name = 'preferred_skills') THEN
    ALTER TABLE public.job_postings 
    ADD COLUMN preferred_skills text[];
  END IF;
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_schema = 'public' 
                   AND table_name = 'job_postings' 
                   AND column_name = 'languages_required') THEN
    ALTER TABLE public.job_postings 
    ADD COLUMN languages_required text[];
  END IF;
...
SET 
  category = COALESCE(category, 'service'),
  position_type = COALESCE(position_type, 'waiter'),
  status = COALESCE(status, 'active')
WHERE category IS NULL OR position_type IS NULL OR status IS NULL;

-- Устанавливаем NOT NULL constraint для обязательных полей (после заполнения данными)
DO $$
BEGIN
  -- Делаем category NOT NULL
  IF EXISTS (SELECT 1 FROM information_schema.columns 
             WHERE table_schema = 'public' 
               AND table_name = 'job_postings' 
               AND column_name = 'category'
               AND is_nullable = 'YES') THEN
    ALTER TABLE public.job_postings ALTER COLUMN category SET NOT NULL;
  END IF;

  -- Делаем position_type NOT NULL  
  IF EXISTS (SELECT 1 FROM information_schema.columns 
             WHERE table_schema = 'public' 
               AND table_name = 'job_postings' 
               AND column_name = 'position_type'
               AND is_nullable = 'YES') THEN
    ALTER TABLE public.job_postings ALTER COLUMN position_type SET NOT NULL;
  END IF;
END $$;

-- Индексы для оптимизации поиска
CREATE INDEX IF NOT EXISTS idx_job_postings_status_city ON public.job_postings(status, city_id) WHERE status = 'active';
CREATE INDEX IF NOT EXISTS idx_job_postings_category_position ON public.job_postings(category, position_type);
CREATE INDEX IF NOT EXISTS idx_job_postings_salary ON public.job_postings(salary_min, salary_max) WHERE salary_min IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_job_postings_created ON public.job_postings(created_at DESC);
CREATE INDEX IF NOT EXISTS idx_job_postings_urgent ON public.job_postings(is_urgent, created_at DESC) WHERE is_urgent = true;
...
DROP POLICY IF EXISTS job_postings_insert_employers ON public.job_postings;
...
-- =============================================================================
-- МИГРАЦИЯ 2: urgent_jobs table (20250913121000)  
-- =============================================================================
...
CREATE INDEX IF NOT EXISTS idx_urgent_jobs_active_deadline ON public.urgent_jobs(status, application_deadline) WHERE status = 'active';
CREATE INDEX IF NOT EXISTS idx_urgent_jobs_date_time ON public.urgent_jobs(needed_date, needed_time_start);
CREATE INDEX IF NOT EXISTS idx_urgent_jobs_city_position ON public.urgent_jobs(city_id, position_type);
CREATE INDEX IF NOT EXISTS idx_urgent_jobs_priority ON public.urgent_jobs(priority, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_urgent_jobs_payment ON public.urgent_jobs(payment_per_shift DESC);
...
-- Удаляем существующие политики если есть
DROP POLICY IF EXISTS urgent_jobs_select_active ON public.urgent_jobs;
DROP POLICY IF EXISTS urgent_jobs_select_own ON public.urgent_jobs;
DROP POLICY IF EXISTS urgent_jobs_insert_employers ON public.urgent_jobs;
DROP POLICY IF EXISTS urgent_jobs_update_own ON public.urgent_jobs;
DROP POLICY IF EXISTS urgent_jobs_delete_own ON public.urgent_jobs;
...
DROP TRIGGER IF EXISTS set_updated_at_urgent_jobs ON public.urgent_jobs;
...
DROP TRIGGER IF EXISTS urgent_jobs_auto_fill ON public.urgent_jobs;
...
WHERE t.table_schema = 'public' 
  AND t.table_name IN ('job_postings', 'urgent_jobs', 'employers')
