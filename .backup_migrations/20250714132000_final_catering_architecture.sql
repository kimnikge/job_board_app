-- Финальная безопасная миграция - создание архитектуры для общепита
-- Дата: 14 июля 2025
...
-- 2. Заполняем справочники (только если пустые)
...
-- 3. Обновляем job_postings для общепита
...
-- 4. Обновляем существующую таблицу urgent_jobs безопасно
alter table urgent_jobs add column if not exists venue_name varchar(200);
...
alter table urgent_jobs add column if not exists address text;
alter table urgent_jobs add column if not exists metro_station varchar(100);
...
alter table urgent_jobs add column if not exists requirements text;
...
alter table urgent_jobs add column if not exists employer_id uuid;
alter table urgent_jobs add column if not exists notification_priority integer default 1;

-- 5. Обновляем applications для поддержки срочных вакансий
...
-- Индексы для urgent_jobs (только если колонки существуют)
do $$
begin
    if exists (select 1 from information_schema.columns where table_name = 'urgent_jobs' and column_name = 'status') then
        create index if not exists idx_urgent_jobs_status on urgent_jobs(status);
    end if;
    
    if exists (select 1 from information_schema.columns where table_name = 'urgent_jobs' and column_name = 'needed_date') then
        create index if not exists idx_urgent_jobs_needed_date on urgent_jobs(needed_date);
    end if;
    
    if exists (select 1 from information_schema.columns where table_name = 'urgent_jobs' and column_name = 'specialization_id') then
        create index if not exists idx_urgent_jobs_specialization on urgent_jobs(specialization_id);
    end if;
    
    if exists (select 1 from information_schema.columns where table_name = 'urgent_jobs' and column_name = 'district_id') then
        create index if not exists idx_urgent_jobs_district on urgent_jobs(district_id);
    end if;
    
    if exists (select 1 from information_schema.columns where table_name = 'urgent_jobs' and column_name = 'employer_id') then
        create index if not exists idx_urgent_jobs_employer on urgent_jobs(employer_id);
    end if;
    
    if exists (select 1 from information_schema.columns where table_name = 'urgent_jobs' and column_name = 'auto_close_at') then
